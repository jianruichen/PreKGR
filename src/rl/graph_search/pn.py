...
class GraphSearchPolicy(nn.Module):
    ...
    def transit(self, e, obs, kg, kg_pred=None, fn_kg=None, use_action_space_bucketing=True, merge_aspace_batching_outcome=False, use_kg_pred=False, inference=False):
        ...
        S2E = self.W3(X)
        S2E = F.relu(S2E)
        S2E = self.W3Dropout(S2E)

        all_relation_embeddings = kg.get_all_relation_embeddings()
        e_r_att = torch.matmul(self.W4(S2E), all_relation_embeddings.t())

        zero_vec = -9e15 * torch.ones_like(e_r_att)
        e_list = e.tolist()
        # 构建邻接矩阵
        for i in range(len(e)):
            if e_list[i] in kg.adj_list:
            # if e_list[i] != 0 and e_list[i] != 1:
                adj_r = list(kg.adj_list[e_list[i]].keys())
                zero_vec[i][adj_r]=1
        e_r_att = torch.where(zero_vec > 0, e_r_att, zero_vec)

        att_weights = torch.nn.functional.softmax(e_r_att, dim=-1)  # 形状为 (batch_size, num_edges)
        # 将注意力系数与邻边特征相乘得到加权的邻边特征
        weighted_edges = torch.matmul(att_weights.unsqueeze(1),
                                      all_relation_embeddings.unsqueeze(0))  # 形状为 (batch_size, num_edges, embedding_dim)

        # 对加权的邻边特征按照节点进行聚合
        aggregated_features = torch.sum(weighted_edges, dim=1)  # 形状为 (batch_size, embedding_dim)

        # 将聚合后的特征与节点表示相加，得到最终的节点表示
        dynamic_E = S2E + aggregated_features

        if self.relation_only:
            X = torch.cat([H, Q], dim=-1)
        elif self.relation_only_in_path:
            E_s = kg.get_entity_embeddings(e_s)
            X = torch.cat([dynamic_E, H, E_s, Q], dim=-1)
        elif use_kg_pred:
            X = torch.cat([dynamic_E, H, Q, kg_pred], dim=-1)
            # X = torch.cat([E, H, Q, kg.get_entity_embeddings(e_t)], dim=-1)
        else:
            X = torch.cat([dynamic_E, H, Q], dim=-1)
        ...
        if use_action_space_bucketing:
            ...
            entropy_list = []
            references = []
            db_action_spaces, db_references = self.get_new_action_space_in_buckets(e, obs, kg, X2=X2, relation_att=relation_att, inference=inference)
    ...
    def get_candidate_actions_embedding(self, candidate_r_space, candidate_e_space, kg):
        candidate_r_embedding = kg.get_relation_embeddings(candidate_r_space)
        candidate_e_embedding = kg.get_entity_embeddings(candidate_e_space)
        candidate_actions_embedding = torch.cat([candidate_r_embedding, candidate_e_embedding], dim=-1)
        return candidate_actions_embedding

    def get_dynamic_action_space(self, e_space, r_space, action_mask, e_b, kg, relation_att, X2_b):
        ...
        relation_att_select_num = additional_relation_size * 3
        # bks x additional_relation_size
        # relation_idx = torch.multinomial(relation_att, additional_relation_size*5)
        relation_idx = torch.multinomial(relation_att, relation_att_select_num)
        # bks x additional_relation_size x |E|
        # S = self.fn.forward(e_b.repeat_interleave(additional_relation_size*5, dim=0), relation_idx.view(bks * additional_relation_size*5), self.fn_kg).view(bks, additional_relation_size*5, self.fn_kg.num_entities)
        S = self.fn.forward(e_b.repeat_interleave(relation_att_select_num, dim=0), relation_idx.view(bks * relation_att_select_num), self.fn_kg).view(bks, relation_att_select_num, self.fn_kg.num_entities)
        # idx -> bks x additional_relation_size x self.avg_entity_per_relation
        _, entity_idx = torch.topk(S, self.avg_entity_per_relation, dim=-1)
        # bks x (additional_relation_size * self.avg_entity_per_relation)
        candidate_r_space = relation_idx.repeat_interleave(self.avg_entity_per_relation, dim=1)
        candidate_e_space = entity_idx.view(bks, -1)

        candidate_A_space = self.get_candidate_actions_embedding(candidate_r_space, candidate_e_space, kg)
        action_atts = torch.nn.functional.softmax(torch.matmul(X2_b[0], candidate_A_space[0].t()), dim=-1)
        action_atts = torch.unsqueeze(action_atts, 0)
        for i in range(1, len(e_b)):
            with torch.no_grad():
                action_att = torch.matmul(X2_b[i], candidate_A_space[i].t())
                action_att = torch.nn.functional.softmax(action_att, dim=-1)
                action_att = torch.unsqueeze(action_att, 0)
            action_atts = torch.cat((action_atts, action_att), dim=0)

        _, action_idx = torch.topk(action_atts, additional_relation_size, dim=-1)

        new_e_space = torch.zeros(bks, additional_relation_size, dtype=torch.int64).cuda()
        new_r_space = torch.zeros(bks, additional_relation_size, dtype=torch.int64).cuda()
        for i in range(bks):
            new_e_space[i] = candidate_e_space[i, action_idx[i]]
            new_r_space[i] = candidate_r_space[i, action_idx[i]]

        new_action_mask = torch.ones(bks, additional_relation_size).cuda()
    ...
    def get_new_action_space_in_buckets(self, e, obs, kg, collapse_entities=False, relation_att=None, X2=None,inference=False):
        """
            ...
            for key in batch_ref:
                ...
                if inference or random.random() < 0.5:
                    if self.use_action_selection:
                        e_space_b, r_space_b, action_mask_b = self.get_dynamic_action_space(e_space_b, r_space_b, action_mask_b, e_b, kg, relation_att[l_batch_refs], X2[l_batch_refs])
    ...
    def define_modules(self):
        ...
        self.W3 = nn.Linear(input_dim, self.entity_dim)
